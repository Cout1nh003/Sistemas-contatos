...
const SESSION_SECRET =
  process.env.SESSION_SECRET || 'orcamentos-session-secret-change-me';
...
function validateContact(body) {
  ...
  if (!contact.nome || contact.nome.length > 120) {
    return { error: 'Informe um nome válido.' };
  }

  if (
    !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(contact.email) ||
    contact.email.length > 160
  ) {
    return { error: 'Informe um email válido.' };
  }

  if (!contact.telefone || contact.telefone.length > 30) {
    return { error: 'Informe um telefone válido.' };
  }

  if (!contact.assunto || contact.assunto.length > 160) {
    return { error: 'Informe um assunto válido.' };
  }

  if (
    !contact.mensagem ||
    contact.mensagem.length > 3000
  ) {
    return {
      error: 'A mensagem deve ter entre 1 e 3000 caracteres.'
    };
  }
        ...
        (selectError, row) => {
          if (selectError) {
            return sendError(
              res,
              500,
              'Contato criado, mas não foi possível retorná-lo.'
            );
          }
...
app.put('/api/contato/:id', requireAuth, (req, res) => {
  ...
  if (!Number.isInteger(id) || id < 1) {
    return sendError(res, 400, 'Contato inválido.');
  }
        ...
        (selectError, row) => {
          if (selectError) {
            return sendError(
              res,
              500,
              'Contato atualizado, mas não foi possível retorná-lo.'
            );
          }
