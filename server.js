...
app.delete('/api/clientes/:id', requireAuth, async (req, res) => { const id = Number.parseInt(req.params.id, 10); if (!Number.isInteger(id) || id < 1) return sendError(res, 400, 'Cliente inválido.'); try { const used = await getQuery('SELECT COUNT(*) AS total FROM orcamentos WHERE clienteId = ?', [id]); if (used.total) return sendError(res, 409, 'Cliente usado em orçamento; não pode ser excluído.'); const result = await runQuery('DELETE FROM clientes WHERE id = ?', [id]); if (!result.changes) return sendError(res, 404, 'Cliente não encontrado.'); res.json({ message: 'Cliente excluído com sucesso.' }); } catch (error) { sendError(res, 500, 'Não foi possível excluir o cliente.'); } });
